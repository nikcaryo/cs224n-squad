...
class CharBiDAF(nn.Module):
    ...
    def forward(self, cw_idxs, qw_idxs, cc_idxs, qc_idxs):
        ...
        c_emb = self.emb(cw_idxs, cc_idxs)   # (batch_size, c_len, word_vectors.size(1) + 2 * char_hidden_size)
        q_emb = self.emb(qw_idxs, qc_idxs)   # (batch_size, q_len, word_vectors.size(1) + 2 * char_hidden_size)
